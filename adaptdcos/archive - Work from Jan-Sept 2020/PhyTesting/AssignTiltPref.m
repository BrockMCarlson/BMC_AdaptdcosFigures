...
warning('assign eye preference below')

tilt1 =  ~STIM.blank &...
    STIM.filen == brfsFileNum &...
    STIM.contrast(:,1) >= .5 &...
    STIM.monocular == 1 & ...
    STIM.eyes(:,1) == 2 & ... %edit each time
    STIM.tilt(:,1) == monoc2tilts(1);
sum(tilt1)
...
legend({'tilt1','tilt2'})
vline(0)
titletext = {strcat('Eye2_TiltSelect_',penetration),strcat('at depth_',depth),strcat('tilt1trlNum=',CondTrialNum{1,:},'___tilt2trlNum=',CondTrialNum{2,:})};
...
if flag_saveFigs
    count = count + 1;
    if count == 1
        export_fig(strcat('Eye2_TiltSelect_',penetration),'-pdf','-nocrop') 
    else
        export_fig(strcat('Eye2_TiltSelect_',penetration),'-pdf','-nocrop','-append')
    end
end
