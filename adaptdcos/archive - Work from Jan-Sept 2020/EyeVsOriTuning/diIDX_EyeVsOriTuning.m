%% diIDX_EyeVsOri
% from pairwise comp - but NOT balanced
% IDX that is eye tuned ONLY
% IDX that is ori tuned only
...
normalize       = 1;
EyeOrOri        = 'eye';
saveName = 'diIDX_Eye_balancedCond';
...
%% Make sure compared conditions are balanced
% Can I do this on trial numbers?

% Or statement -- 2x2 adaptor plot concern only?
% Must have DExPS(5A)-NDExNS(5S) AND NDExNS(8A)-DExPS(8S)
%%%% OR
% Must have DExNS(6A?)-NDExPS(6S?) AND NDExPS(7A?)-DExNS(7S?)

if (CondTrialNumS(5) > 0 && CondTrialNumS(8) > 0) ||...
        (CondTrialNumS(6) > 0 && CondTrialNumS(7) > 0)
    LFG = LFG+1;
else
    continue
end
