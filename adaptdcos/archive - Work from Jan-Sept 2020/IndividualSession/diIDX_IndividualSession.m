%% diIDX_IndividualSession

% Correct adaptor and suppresor
% Monocular 2x2
% half contrast everything
% no subtractions - no respone vector, just all adapted responses and 2x2
% SDF s
...
flag_saveIDX    = 1;
normalize       = 0;
saveName = 'diIDX_IndividualSession';
...
CI_S = nan(size(conditionarrayS,1),size(sdf,1));
CI_A = nan(size(conditionarrayS,1),size(sdf,1));
...
for cond = 1:size(conditionarrayA,1)
    ...
    for x = 1:2
        ...
        SEM = std(sdf(:,shift),[],2)/sqrt(sum(shift));	% Standard Error
        ts = tinv(0.99,sum(shift)-1);                   % T-Score at the 99th percentile
        CI = nanmean(sdf(:,shift),2) + ts*SEM;          % Confidence Intervals
        if x == 1
            CI_uncropA(cond,:) = CI;
        else
            CI_uncropS(cond,:) = CI;
...
%% crop/pad SDF
        ...
        pad8rows = repmat(pad,8,1); % pads with NANs if you are not in that condition any more. 
            ...
            CI_A = cat(2,CI_uncropA(:, st : en,:), pad8rows); clear CI_uncropA; 
            CI_S = cat(2,CI_uncropS(:, st : en,:), pad8rows); clear CI_uncropS; 
            
            SDF_monoc       = cat(2,SDF_monoc_uncrop(:, st : en,:), pad4rows); clear SDF_monoc_uncrop; 
            CI_monoc = cat(2,CI_monoc_uncrop(:, st : en,:), pad4rows); clear CI_monoc_uncrop; 
...
%% SAVE
if flag_saveIDX
    cd('C:\Users\Brock\Documents\MATLAB\GitHub\ephys-analysis\dichoptic\adaptdcos\IndividualSession')
    if isfile(saveName)
