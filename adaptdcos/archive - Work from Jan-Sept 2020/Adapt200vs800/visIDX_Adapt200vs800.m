%% visIDX_Adapt200vs800
% from visIDX_EffectOfAdap
...
clear balance200 balance800
balance200 = ~any((CondTrialNum_SDF(:,1:4) == 0),2);
balance800 = ~any((CondTrialNum_SDF(:,[1 2 5 6]) == 0),2);
...
for i = [1,2,5,6]
    count = count+1;
p(count) = plot(TM,SDFavg_800(i,:),colorMain{count},'LineWidth',2); hold on
plot(TM,VARline_Up_800(i,:),colorVar{count},'LineWidth',1); hold on
plot(TM,VARline_Down_800(i,:),colorVar{count},'LineWidth',1); hold on
ylim([-.1 1]);
xlim([-.05 .35]);
end
vline(0)
legend([p(1) p(2) p(3) p(4)],conditName{[1 2 5 6],:})
...
ylabel({'Change in spiking, unadapted - adapted','FeatureScaled'})
ylim([-.5 .5])
% xlim([-.5 .5])
xlabel('Change in contrast, DE/NDE')
