function AUTOdiIDX_LowLowContrast
...
saveName = 'diIDX_AUTO_LowLowContrast'; % THIS IS CONTRAST LEVELS OF .1-.4 INCLUSIVE
anaType = '_AUTO.mat';
flag_saveIDX    = 1;
...
for cond = 1:size(conditionarray,1)
    ...
    if cond < 5
      ...
      trls = I &...
            ...
            (STIM.contrast(:,1)  >= .1 & STIM.contrast(:,1) <= .3);
%         
%         trls = I &...
...
%             (SORTED.contrasts(:,1)  >= .9 & SORTED.contrasts(:,1) <= 1);
    ...
    elseif cond == 5 && cond == 6 % get simultaneous trials
        trls = I &...
            ...
            STIM.soa       == conditionarray(cond,5) & ...
            STIM.monocular == conditionarray(cond,6) & ...
            ((SORTED.contrasts(:,1)  >= .1) & (SORTED.contrasts(:,1)  <= .3 )) &...
            ((SORTED.contrasts(:,2)  >= .1) & (SORTED.contrasts(:,2)  <= .3 ));
        trlsLogical(:,cond) = trls;
        CondTrials{cond} = find(trls);
        CondTrialNum_SDF(cond,1) = sum(trls); 
        SDF_uncrop(cond,:)   = nanmean(sdf(:,trls),2);    % Use trls to pull out continuous data     
