function gramm_2x2_subline_NSNDEvariation(IDX)
%% Goal
...
for condLabel = [1 2]
   for uct = 1:uctLength
       ...
       % DV - SDF
       if condLabel == 1
           if isempty(IDX.allV1(uct).SDF_avg{6}) || ...
                   any(isnan(IDX.allV1(uct).SDF_avg{6}(1:560))) ||...
                   isempty(IDX.allV1(uct).SDF_avg{7})||...
                   any(isnan(IDX.allV1(uct).SDF_avg{7}(1:560)))
               count = count - 1;
           else
               congSDF = IDX.allV1(uct).SDF_avg{6}(1:560)';
               inCoSDF = IDX.allV1(uct).SDF_avg{7}(1:560)';
               subSDF = congSDF - inCoSDF;
               DataForVis.SDF{count,1} = subSDF;
           end
       elseif condLabel == 2
           if isempty(IDX.allV1(uct).SDF_avg{12}) || ...
                   any(isnan(IDX.allV1(uct).SDF_avg{12}(1:560))) ||...
                   isempty(IDX.allV1(uct).SDF_avg{20})||...
                   any(isnan(IDX.allV1(uct).SDF_avg{20}(1:560)))
               count = count - 1;
           else
               congSDF = IDX.allV1(uct).SDF_avg{12}(1:560)';
               inCoSDF = IDX.allV1(uct).SDF_avg{20}(1:560)';
               subSDF = congSDF - inCoSDF;
               DataForVis.SDF{count,1} = subSDF;
           end
       end
