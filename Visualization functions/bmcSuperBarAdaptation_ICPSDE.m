function bmcSuperBarAdaptation_ICPSDE(IDX)
...
for i = 1:6
   ...
   RESPsem.transient(i,1) = (nanstd(RESPall.transient(i,:),[],2)./sqrt(size(RESPall.transient,2)));
   RESPavg.sustained(i,1) = nanmean(RESPall.sustained(i,:),2);  
   RESPsem.sustained(i,1) = (nanstd(RESPall.sustained(i,:),[],2)./sqrt(size(RESPall.sustained,2)));
end


dv = {[1 2],[2 4],[1 4],[2 4],[5 6]};
for j = 1:5
...
Y = [RESPavg.transient(1), RESPavg.transient(2), RESPavg.transient(4);
     RESPavg.sustained(1), RESPavg.sustained(2), RESPavg.sustained(4)];
E = [RESPsem.transient(1), RESPsem.transient(2), RESPsem.transient(4);
     RESPsem.sustained(1), RESPsem.sustained(2), RESPsem.sustained(4)];
C = [.8 .2 .2;
     .2 .2 .8];
...
E = [RESPsem.transient(2), RESPsem.transient(4), RESPsem.transient(5), RESPsem.transient(6);
     RESPsem.sustained(2), RESPsem.sustained(4), RESPsem.sustained(5), RESPsem.sustained(6)];
C = [.8 .2 .2;
     .2 .2 .8];
